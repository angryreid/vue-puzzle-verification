<template>
  <div ref="validate_box">
    ...
    <div class="this_div" ref="this_div">
      ...
      <div ref="hgroup" class="hgroup"></div>
      <div ref="this_msg_error" class="this_msg_error"></div>
      <div ref="this_msg_ok" class="this_msg_ok"></div>
      <div class="slide">
...
export default {
  data() {
    return {
      // 以 _ 或 $ 开头**的属性
      // 不会被 Vue 实例代理，因为它们可能和 Vue 内置的属性、API 方法冲突。你可以使用例如 **vm.$data._property 的方式访问这些属性
      validate_img: null,
      validate_img_loaded: false,
      validate_is_draw_bg: false,
      validate_is_moving: false,
      validate_block_start_x: 0,
      validate_block_start_y: 0,
      validate_doing: false,
      validate_mark_w: 50,
      validate_mark_h: 50,
      validate_mark_offset: 0,
      validate_img_w: 240,
      validate_img_h: 150,
  ...
  methods: {
    ...
    _block_start_move(e) {
      ...
      this._draw_bg();
      this.validate_block_start_x = theEvent.clientX;
      this.validate_block_start_y = theEvent.clientY;
      this.validate_doing = true;
      this.validate_is_moving = true;
    },
    _block_on_move(e) {
      ...
      var offset = theEvent.clientX - this.validate_block_start_x;
      if (offset < 0) {
        offset = 0;
      }
      var max_off = this.validate_img_w - this.validate_mark_w;
      if (offset > max_off) {
        offset = max_off;
      }
      ...
      this.validate_mark_offset =
        (offset / max_off) * (this.validate_img_w - this.validate_mark_w);
      this._draw_bg();
      this._draw_mark();
    },
    _block_on_end(e) {
      ...
      this.validate_is_moving = false;
      this._sendvalidate_result();
    },
    _sendvalidate_result() {
      var haddle = {
        success: this._sendvalidate_result_success,
        failure: this._sendvalidate_result_failure
      };
      ...
      // re.request(
      //   "get",
      //   this._currentUrl() + "check.php?tn_r=" + this.validate_mark_offset,
      //   haddle
      // );
    },
    _sendvalidate_result_success(responseText, responseXML) {
      this.validate_doing = false;
      if (responseText == "ok") {
        ...
        this._showmsg("验证失败");
        this.validate_err_c++;
        if (this.validate_err_c > this.single_img_check_time) {
          this.refresh();
        }
      }
    },
    _sendvalidate_result_failure(xhr, status) {},
    _draw_fullbg() {
      let canvas_bg = this.$refs.this_canvas_bg;
      let ctx_bg = canvas_bg.getContext("2d");
      ctx_bg.drawImage(
        this.validate_img,
        0,
        this.validate_img_h * 2,
        this.validate_img_w,
        this.validate_img_h,
        0,
        0,
    ...
    _draw_bg() {
      if (this.validate_is_draw_bg) {
        return;
      }
      this.validate_is_draw_bg = true;
      let canvas_bg = this.$refs.this_canvas_bg;
      let ctx_bg = canvas_bg.getContext("2d");
      ctx_bg.drawImage(
        this.validate_img,
    ...
    _draw_mark() {
      ...
      let imageData = ctx_mark.getImageData(
        0,
        0,
        this.validate_img_w,
        this.validate_img_h
      ...
      //alert(data.length/4);
      var x = this.validate_img_h,
        y = this.validate_img_w;
      for (var j = 0; j < x; j++) {
        var ii = 1,
          k1 = -1;
    ...
    _showmsg(msg, status) {
      ...
      if (!status) {
        status = 0;
        obj = this.$refs.this_msg_error;
      } else {
        obj = this.$refs.this_msg_ok;
      }
      obj.innerHTML = msg;
      let setOpacity = (ele, opacity) => {
    ...
    refresh() {
      ...
      this.validate_img.src = img_url;
      this.validate_img.onload = () => {
        // this._draw_fullbg();
        var canvas_mark = this.$refs.this_canvas_mark;
        var ctx_mark = canvas_mark.getContext("2d");
        //清理画布
        ctx_mark.clearRect(0, 0, canvas_mark.width, canvas_mark.height);
        this.validate_img_loaded = true;
